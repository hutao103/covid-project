<#include "inc/header.ftl">
        <div class="right">
            <div class="location">
                ...
                <span>MONITOR MANAGER</span>
            </div>
            <div class="search">
           		<form method="get" action="${request.contextPath}/monitor/query">
            ...
            <table class="providerTable" cellpadding="0" cellspacing="0">
                <tr class="firstTr">
                	...
                	<#list monitorList as monitor>
                		<tr>
							<td><span>${monitor.userName?default('')}</span></td>
							<td><span>${monitor.addressName?default('')}</span></td>
							<td><span>${monitor.startTime?default('')}</span></td>
							<td><span>${monitor.createTime?string('yyyy-MM-dd HH:mm')}</span>
						</tr>
                	</#list>
			</table>
